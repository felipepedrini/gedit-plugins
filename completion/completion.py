...
{Enhance for python by oeway<oeway007@gmail.com>,Thanks to another plugin named "Python Code Completion" by Fenrrir.}
...
class CompletionWindow(gtk.Window):
    ...
    def __init__(self, parent):
        ...
        self.hbox = gtk.HBox()
        self.scroller_text=gtk.ScrolledWindow()
    ...
    def _init_containers(self):
        ...
        hbox.add(scroller)
        #frame.add(scroller)
        scroller_text = self.scroller_text
    ...
    def select_next(self):
        ...
        if row in self.info_dict:
            self.text_buffer.set_text(self.info_dict[row])
...
#        self.completions.reverse()
        ...
        #self.text_buffer.set_text(self.info_dict[0])
        if 0 in self.info_dict:
            self.text_buffer.set_text(self.info_dict[0])
...
class CompletionPlugin(gedit.Plugin):
    ...
    def __init__(self):
        ...
        self._completions = []
        self._completes=None
        self._favorite_words = {}
        self._font_ascent = 0
        self._remains = []
        self._incomplete_num=0
        self._user_keywords=[]
        self._load_user_keywords()
    def _load_user_keywords(self):
        try:
            kwpath=os.path.join(os.path.expanduser("~/.gnome2/gedit/plugins/"), "completion/user_keywords.conf")
            kf=open(kwpath,"r")
            while True:
                line=kf.readline()[:-1]
                if len(line)==0:
                   break
                self._user_keywords.append(line)
        except:
            pass
	...
	#_all_words.update(keyword.kwlist)
	_all_words.update(self._user_keywords)
        limit = self._max_completions_to_show
        incomplete=incomplete.lower()
        for sequence in (favorites, _all_words):
            for word in sequence:
		word_l=word.lower()
                if not word_l.startswith(incomplete): continue
                if word_l == incomplete: continue
                if word in self._completions: continue
                self._completions.append(word)
                self._remains.append(word[:])#[len(incomplete):])
                if len(self._remains) >= limit and not incomplete=="__" : break
        window = gedit.app_get_default().get_active_window()
        completion_window = self._completion_windows[window]
        #if self._completes:
        completion_window.info_dict.clear()
        addded=False
        for completio in self._completes:
            word=completio['abbr']
            if word in self._completions: continue
            self._completions.append(word)
            self._remains.append(word[:])#[len(incomplete):])
            completion_window.info_dict[len(self._remains)-1]=completio['info']
            if len(self._remains) >= limit and not incomplete=="__" : break
            addded=True
        if addded:
        #try:
            completion_window.hbox.add(completion_window.scroller_text)
        else:
            #try:
            completion_window.hbox.remove(completion_window.scroller_text)
    ...
    def _on_view_key_press_event(self, view, event, window):
        ...
        if event.state & gtk.gdk.MOD1_MASK:
            return self._terminate_completion()
        if ((event.keyval == gtk.keysyms.Tab) or (event.keyval == gtk.keysyms.Right) )and self._remains:
            return not self._complete_current(view)
        completion_window = self._completion_windows[window]
